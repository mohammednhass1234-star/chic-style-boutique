    ...
    return (
        <div className="container" dir={dir}>
            <header className={styles.sectionHeader} style={{ marginTop: '8rem', marginBottom: '6rem' }}>
                <h1 className="elegant-text" style={{ fontSize: '4rem', color: 'var(--dark-charcoal)' }}>{language === 'ar' ? 'أزياء الأطفال' : 'Mode Enfants'}</h1>
                <div className="line-separator" style={{ margin: '2rem auto' }}></div>
                <p style={{ fontSize: '1.2rem', color: 'var(--text-muted)' }}>{language === 'ar' ? 'تشكيلة مبهجة ومريحة تليق بصغاركم' : 'Une collection joyeuse et confortable pour vos petits'}</p>
            </header>
                ...
                <div className={styles.productGrid} style={{ marginBottom: '8rem' }}>
                    {products.length > 0 ? (
                        products.map(product => {
                            ...
                            return (
                                <Link key={product.id} href={`/product/${product.id}`} className={styles.placeholderCard}>
                                    {product.isOfferActive && <div className={styles.saleBadge}>{language === 'ar' ? 'حصري' : 'Promo'} {discount > 0 && `${discount}%`}</div>}
                                    <div
                        ...
                        <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '10rem', fontSize: '1.2rem', color: 'var(--text-muted)' }}>
                            {language === 'ar' ? 'لا توجد منتجات متوفرة حالياً في هذا القسم. ترقبوا تشكيلتنا الجديدة للأطفال.' : 'Aucun produit disponible dans cette section pour le moment. Restez à l\'écoute pour notre nouvelle collection.'}
                        </div>
                    )}
                </div>
            )}
